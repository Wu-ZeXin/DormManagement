...
router.post('/adduser', async (ctx, next) => {
  ...
  let insertMysql = await query(
    `insert into user(uname,pass,name,sex,xueyuan,banji,tel,sushe,lynumber,fdy,fdytel,state,roler) values('${uname}','${pass}','${name}','${sex}','${xueyuan}','${banji}','${tel}','${sushe}','${lynumber}','${fdy}','${fdytel}',1,2)`
  );
...
router.post('/edit', async (ctx, next) => {
  ...
  var xueyuan = ctx.request.body.xueyuan;
  var banji = ctx.request.body.banji;
  let a = await query(`update user set
    name = '${name}',
    uname = '${uname}',
    tel = '${tel}',
    fdy = '${fdy}',
