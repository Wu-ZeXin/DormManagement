...
router.post('/add', async (ctx, next) => {
	...
	let bxdate = ctx.request.body.date;
	...
	// 将信息添加到数据库
	let a = await query(
		`insert into bxlb(uname,tel,lynumber,sushe,bxdate,bxtime,bxdel,state,switchon) values('${uname}','${tel}','${lynumber}','${sushe}','${bxdate}','${bxtime}','${bxdel}','${state}','${switchon}')`
	);
	console.log(a);
	if (a.affectedRows == 1) {
		// 代表插入成功
